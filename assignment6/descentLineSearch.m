...
while (~stopCond && nIter <= maxIter)
  ...
  % Increment iterations
    ...
    switch lower(descent)
      ...
      case 'newton-cg'
        %=========================== YOUR CODE HERE ======================================
        z_j = zeros(size(x_k)); r_j = F.df(x_k); d_j = -r_j;
        B_k = F.d2f(x_k);
        newton_cg_tol = min(0.5,sqrt(norm(F.df(x_k))))*norm(F.df(x_k));
        for j=0:maxIter/2
            if d_j'*B_k*d_j <= 0
               if j==0
                  p_k = -F.df(x_k);
            ...
            z_j_plus_1 = z_j + alpha_j*d_j;
            r_j_plus_1 = r_j + alpha_j*B_k*d_j;
            if norm(r_j_plus_1) < newton_cg_tol
               p_k = z_j_plus_1;
               break;
            end
            ...
            %set new values
            z_j = z_j_plus_1; r_j = r_j_plus_1; d_j = d_j_plus_1; 
        end
        %=================================================================================  
      case 'bfgs'
        p_k = -H_k(F.df(x_k));
